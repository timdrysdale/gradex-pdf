...
func getTextFieldsActionBar(pageNum int, verb string) []TextField {
	...
	fields = append(fields, TextField{ID: "pageOK", Bound: Box{TopLeft: Point{X: 41.3 * mm, Y: (312 - 47.0) * mm}, Width: 8 * mm, Height: 8 * mm}})

	// get Q box

	qbox1 := getTextFieldsQ()

	qbox1 = displaceTextFields(qbox1, 21.7*mm, (312-67.4)*mm)
	...
	qbox2 = displaceTextFields(qbox2, 21.7*mm, (312-137.1)*mm)
	...
	fields = append(fields, TextField{ID: "taskOK", Bound: Box{TopLeft: Point{X: (21.3 + 19.8) * mm, Y: (312 - 286.4) * mm}, Width: 8 * mm, Height: 8 * mm}})
...
func addLadderTextFields(page *model.PdfPage, form *model.PdfAcroForm, ladder Ladder) {
	...
	for _, box := range ladder.Boxes {
		...
		rect := []float64{box.Bound.TopLeft.X, box.Bound.TopLeft.Y, box.Bound.TopLeft.X + box.Bound.Width, box.Bound.TopLeft.Y - box.Bound.Height}
